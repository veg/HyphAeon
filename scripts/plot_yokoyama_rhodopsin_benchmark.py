...
# Create figure
fig = plt.figure(figsize=(15.5, 11.6), dpi=300)
gs = fig.add_gridspec(3, 1, height_ratios=[1.05, 0.36, 1.15], hspace=0.24)
...
ax1.scatter(sites[background], rhos[background], s=16, color='#94a3b8', alpha=0.5,
            edgecolor='none', label=f'Background Sites (q > 0.05, N = {sum(background)})')

# 2. Unclustered significant
ax1.vlines(sites[sig_unclustered], 0, rhos[sig_unclustered], color='#a855f7', lw=1.3, alpha=0.85)
ax1.scatter(sites[sig_unclustered], rhos[sig_unclustered], s=36, facecolor='#c084fc', edgecolor='#7e22ce', lw=0.9,
            zorder=4, label=f'Unclustered Significant Sites (q \u2264 0.05, N = {sum(sig_unclustered)})')

# 3. Sector 1 (Green)
ax1.vlines(sites[mask_s1], 0, rhos[mask_s1], color='#16a34a', lw=1.6, alpha=0.95)
ax1.scatter(sites[mask_s1], rhos[mask_s1], s=48, facecolor='#22c55e', edgecolor='#14532d', lw=1.1,
            zorder=5, label=f'Sector 1: Extracellular Cap (N = {sum(mask_s1)})')

# 4. Sector 2 (Blue)
ax1.vlines(sites[mask_s2], 0, rhos[mask_s2], color='#2563eb', lw=1.6, alpha=0.95)
ax1.scatter(sites[mask_s2], rhos[mask_s2], s=48, facecolor='#3b82f6', edgecolor='#1e3a8a', lw=1.1,
            zorder=5, label=f'Sector 2: Schiff Base Core (N = {sum(mask_s2)})')

# 5. Sector 3 (Red)
ax1.vlines(sites[mask_s3], 0, rhos[mask_s3], color='#dc2626', lw=1.6, alpha=0.95)
ax1.scatter(sites[mask_s3], rhos[mask_s3], s=48, facecolor='#ef4444', edgecolor='#7f1d1d', lw=1.1,
            zorder=5, label=f'Sector 3: Schiff Base Ceiling (N = {sum(mask_s3)})')

# 6. Sector 4 (Orange)
ax1.vlines(sites[mask_s4], 0, rhos[mask_s4], color='#ea580c', lw=1.6, alpha=0.95)
ax1.scatter(sites[mask_s4], rhos[mask_s4], s=48, facecolor='#f97316', edgecolor='#7c2d12', lw=1.1,
            zorder=5, label=f'Sector 4: Pressure Clamp (N = {sum(mask_s4)})')

# FDR significance cutoff
ax1.axhline(fdr_threshold, color='#dc2626', linestyle='--', linewidth=1.2, alpha=0.85,
...
ax1.set_ylabel(r'Trait Directional Concordance ($\rho_s$)', fontweight='bold', fontsize=12.5)
ax1.set_title('(A) Trait Directional Concordance Across 38 Vertebrate Rhodopsins (Color-Coded by Sector Membership)',
              fontweight='bold', loc='left', pad=10, fontsize=13.5)
ax1.legend(loc='upper left', frameon=True, fontsize=10.0, framealpha=0.95, ncol=2)
plt.setp(ax1.get_xticklabels(), visible=False)
ax1.tick_params(direction='out', length=4.5, width=1.1, labelsize=11.0)

# =============================================================
# Track: Rhodopsin Gene Schematic & Experimental Annotations
# =============================================================
ax_gene.set_ylim(-1.45, 1.05)
...
ax_gene.add_patch(patches.Rectangle((1, bar_y), 329, bar_h,
                                    facecolor='#f1f5f9', edgecolor='#94a3b8', lw=1.0, zorder=2))
...
for start, end, label in tm_domains:
    rect = patches.Rectangle((start, bar_y), end - start, bar_h,
                             facecolor='#334155', edgecolor='#0f172a', lw=1.1, zorder=3)
    ax_gene.add_patch(rect)
    ax_gene.text((start + end) / 2, bar_y + bar_h/2, label, fontsize=8.2, fontweight='bold',
                 color='white', ha='center', va='center', zorder=4)

ax_gene.text(17, bar_y + bar_h/2, 'N-term', fontsize=8.0, fontweight='bold', color='#64748b', ha='center', va='center', zorder=3)
ax_gene.text(320, bar_y + bar_h/2, 'C-term', fontsize=8.0, fontweight='bold', color='#64748b', ha='center', va='center', zorder=3)

ax_gene.text(-4, bar_y + bar_h/2, 'Gene Model:\n(RH1)', fontsize=9.5, fontweight='bold',
...
for site, aa_mut, shift, c in known_spectral:
    ax_gene.plot([site, site], [bar_y, bar_y + bar_h], color='#f59e0b', lw=2.4, zorder=5)
    ax_gene.scatter([site], [bar_y], s=28, facecolor=c, edgecolor='white', lw=1.0, zorder=6)
    
    target_y = tiers.get(site, -0.35)
    ax_gene.plot([site, site], [bar_y, target_y + 0.14], color=c, lw=1.1, ls=':', zorder=4)
    
    badge_str = f'{aa_mut}\n({shift})'
    ax_gene.text(site, target_y, badge_str, fontsize=8.8, fontweight='bold', color=c,
                 ha='center', va='top',
                 bbox=dict(boxstyle='round,pad=0.22', facecolor='white', edgecolor=c, lw=0.9, alpha=0.95),
                 zorder=7)

ticks = [1, 50, 100, 150, 200, 250, 300, 330]
for t in ticks:
    ax_gene.plot([t, t], [bar_y + bar_h, bar_y + bar_h + 0.07], color='#334155', lw=1.1)
    ax_gene.text(t, bar_y + bar_h + 0.11, str(t), fontsize=9.0, color='#334155', ha='center', va='bottom')
ax_gene.text(165, bar_y + bar_h + 0.30, 'Codon Position along Rhodopsin (RH1)', fontsize=10.8,
...
ax2.set_title(r'(B) Four Epistatic Sectors Extracted via Modularity Mining',
              fontweight='bold', loc='left', pad=12, fontsize=13.5)
...
for card in sector_cards:
    ...
    rect = patches.FancyBboxPatch((bx, by), bw, bh, boxstyle='Round,pad=0.5,rounding_size=1.5',
                                  facecolor=card['color'], edgecolor=card['ec'], lw=1.4, zorder=1)
    ax2.add_patch(rect)
    
    ax2.text(bx + bw/2, by + bh - 5.5, card['id'], fontsize=10.5, fontweight='bold',
    ...
    for n1, n2, w in card['edges']:
        if n1 in card['nodes'] and n2 in card['nodes']:
            ...
            ax2.plot([x1, x2], [y1, y2], color='#64748b', lw=w*0.85, alpha=0.75, zorder=3)
            mx, my = (x1 + x2)/2, (y1 + y2)/2
            ax2.text(mx, my, f'{w:.2f}', fontsize=8.0, fontweight='bold', ha='center', va='center',
                     bbox=dict(boxstyle='round,pad=0.18', facecolor='white', edgecolor='#cbd5e1', lw=0.6), zorder=4)
            
    for name, (x, y, bg_c, txt_c) in card['nodes'].items():
        circle = plt.Circle((x, y), 3.3, facecolor=bg_c, edgecolor='black', lw=1.2, zorder=5)
        ax2.add_patch(circle)
        ax2.text(x, y, name, color=txt_c, fontsize=8.8, fontweight='bold', ha='center', va='center', zorder=6)
...
output_paths = [
    ...
    '/Users/sergei/Projects/TOGA_MEME/science_paper/figures/yokoyama_rhodopsin_benchmark.pdf',
    '/Users/sergei/Projects/TOGA_MEME/science_paper/figures/yokoyama_rhodopsin_benchmark.png',
]
...
plt.close()
print('Successfully saved updated figure to PDF and PNG in both paper directories!')
