"""
chronaeon/dating_models.py
--------------------------
Molecular clock estimators for ancestor dating (t_MRCA).

Implements:
- Centered Root-to-Tip Ordinary Least Squares (OLS / TempEst emulation
  with delta-method & bootstrap CIs).
- HyphAeon Attention-Derived Phylogenetic Generalized Least Squares
  (PGLS) via A_fused covariance, with REML Pagel's lambda estimation.
- Non-Linear Restricted Cubic Spline Clock (2 DF) and Power-Law Clock
  with hypothesis testing.
- Confidence intervals: Fieller's theorem, Poisson, and residual
  bootstrap.
- Leave-One-Out Cross-Validation (LOOCV) for tip-date prediction.

Shared numerical helpers (_spectral_eigh, _gls_fit, _generalized_r2)
centralize the spectral decomposition, GLS solve, and Buse R² logic
used across the PGLS / spline / power-law estimators.
"""
