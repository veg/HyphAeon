...
| `preprocess/assemble.js` | `load_alignment_and_tree` end to end (`dataset.py:573-747`) as `loadAlignmentAndTree(alignmentText, treeText, options)`: the tree path, and the tree-free `use_tn93` path (`dataset.py:598-636`) taken on request, when no tree is given, or when the tree has no usable branch lengths (D22; `notices.treeFree.reason` says which, `tree` is then the parsed tree for display or null); plus `siteBatch` / `siteBatches` / `batchSizeFor` for the runtime's tensor bundles |
...
(`dataset.py:224-287`) — a tree without usable branch lengths goes tree-free instead, and
`needsBranchLengths(tree)` / `notices.branchLengthsMissing` still report the fact; the compiled
`tn93` binary the reference prefers (`dataset.py:505-537`; `tn93.js` is the package algorithm,
measured identical); the Lanczos MDS path for N > 500 (`dataset.py:360-381`; the library always
...
);   // loaded.tree is null on a tree-free load: permulations_count 0, gene_p_value_perm null, as phenotype.py:426
