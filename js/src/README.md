...
| `index.js` | the single public entry point; `package.json`'s `exports` map exposes nothing else; `test/index.test.js` pins the 256 public names |
...
Python); the neural BUSTED head; everything of `hyphaeon/temporal.py` and `hyphaeon/dating.py` past
their date parsers — the root-to-tip regression, the clock and the temporal attribution regression
(`dates.js` is PLAN-TEMPORAL phase 2, the date layer only). Every method module of `hyphaeon/*.py`
