 ...
 * veg/HyphAeon 61d30e3 (tag phase-2a):
 *
 *   computePhylogeneticCovariance   compute_phylogenetic_covariance, phenotype.py:275-298
 *   generatePermulations            generate_permulations,           phenotype.py:299-346
 *
 ...
 * `run_phenotype_association` says the same thing at phenotype.py:426 (`permulations > 0 and
 ...
 *      catches it and drops the permulations (phenotype.py:441-443), and so does
...
/**
 * `compute_phylogenetic_covariance(tree, taxa)` (phenotype.py:275-298).
 *
...
export function computePhylogeneticCovariance(tree, taxa) {
	...
	// phenotype.py:283 — {t.name: distance(root, t) for t in get_terminals()}; a repeated name
...
/**
 * `generate_permulations(original_y, tree, taxa, n_perm=1000, seed=42)` (phenotype.py:299-346).
 *
 * `permMatrix` is [n_perm, M] row-major float64. TREE-FREE MODE: the reference only calls this
 * when `tree_obj is not None` (phenotype.py:426), so a null `tree` here is not an error — it
...
export function generatePermulations(y, tree, taxa, options = {}) {
	...
	// phenotype.py:328-330 — np.unique(original_y): sorted distinct values.
	const uniq = new Set();
	...
	if (tree === null || tree === undefined) {
		return skip(
			'no-tree',
			'phenotype.py:426 runs permulations only when tree_obj is not None; in tree-free mode ' +
	...
	// phenotype.py:323 — cholesky(V + 1e-7 * I). The ridge is added to a copy; V itself is
	...
	// phenotype.py:326 — np.random.randn(M, n_perm), C-order, then Z = L @ R.
	...
	if (isBinary) {
		for (let p = 0; p < nPerm; p++) {
			...
			// QUIRK (phenotype.py:334): `[-k:]` with k == 0 is `[0:]`, the whole column.
